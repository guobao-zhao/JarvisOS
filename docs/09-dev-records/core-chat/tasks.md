...
- Step 3 最小实现：在 `dependencies` 中新增 `"ai": "^4.3.0"` 和 `"@ai-sdk/anthropic": "^1.2.0"`
...
- Step 3 最小实现：使用 `createAnthropic` + Kimi baseURL + `KIMI_API_KEY` 环境变量，实现 `streamChat(messages, onText, onError)`，用 `streamText` 流式输出
- Step 4 验证：`bun run typecheck` 通过
- DoD：函数签名与设计一致，类型无报错
